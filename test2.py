...
# remove 1 encoder/decoder
# reduce number of channel
...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.relu(x + self.conv5(out))
        return out
...
class Decoder4(nn.Module):
    def __init__(self):
        ...
        self.convblock = nn.Sequential(
            convrelu((24*2)+1, 24*2),  
            ResBlock(24*2, 6),
            nn.ConvTranspose2d(24*2, 18 + 4, 4, 2, 1)
        )
...
class Decoder2(nn.Module):
    def __init__(self):
        ...
        self.convblock = nn.Sequential(
            convrelu((18 * 2) + 4, 18 * 2), 
            ResBlock(18 * 2, 6), 
            nn.ConvTranspose2d(18 * 2, 12 + 4, 4, 2, 1, bias=True)
        )

    def forward(self, ft_, f0, f1, up_flow0, up_flow1):
        f0_warp = warp(f0, up_flow0)
        f1_warp = warp(f1, up_flow1)
        f_in = torch.cat([f0_warp, f1_warp, up_flow0, up_flow1], 1)
        f_out = self.convblock(f_in)
        return f_out
...
class Decoder1(nn.Module):
    def __init__(self):
        ...
        self.convblock = nn.Sequential(
            convrelu((12*2)+4, 12*2),  
            ResBlock((12*2), 6), 
            nn.ConvTranspose2d((12*2), 8, 4, 2, 1, bias=True)
        )
...
class Model(nn.Module):
    ...
    def forward(self, img0, img1, embt, imgt, flow=None):
        img0 = self.quant(img0)
        img1 = self.quant(img1)
        imgt = self.quant(imgt)
        ...
        imgt_pred = self.dequant(imgt_pred)
...
# Example shapes: adjust depending on your dataset
batch_size = 1
channels = 3
...
img0 = (batch_size, channels, height, width)
img1 = (batch_size, channels, height, width)
embt = (batch_size, 1,1,1)  # guessing embedding is 1D (adjust if wrong)
imgt = (batch_size, channels, height, width)
